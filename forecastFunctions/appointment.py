...
# Function for appointments forecasting monthly or weekly


def handle_appointment_forecast(event, collection):
    try:
...
# Function for cancel appointments (no shows) forecasting monthly or weekly


def handle_noshows_forecast(event, collection):
    try:
        items = list(collection.find({}, {'_id': 0}))
        # Convert list of dictionaries to DataFrame
        df = pd.DataFrame(items)
...
# Function for appointments forecasting by doctors monthly or weekly


def handle_appointment_doctor_forecast(event, collection):
    try:
        items = list(
...
# Function for appointments forecasting by appontment types monthly or weekly


def handle_appointment_type_forecast(event, collection):
    try:
        items = list(
            collection.aggregate(
                [
