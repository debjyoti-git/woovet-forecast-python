...
def handle_client_forecast(request):
    try:
        _db = get_db_instance()
        ...
        collection = _db["forecast_clients"]
        items = list(collection.find({}, {"_id": 0}).limit(5))
        df = pd.DataFrame(items)
        # Rename the columns
        df.columns = ["ds", "y"]
        # return generate_response(200,duration)
        ...
        if interval == "weekly":
            ...
            return {"body": json.dumps(weekly_response)}
        else:
